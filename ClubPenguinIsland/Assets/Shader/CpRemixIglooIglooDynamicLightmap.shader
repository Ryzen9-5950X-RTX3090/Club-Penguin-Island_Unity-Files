		...
		Pass {
			...
			Program "fp" {
				SubProgram "d3d11 " {
					...
					void main()
					{
					    u_xlat0 = texture(_Lightmap, vs_TEXCOORD1.xy);
					    u_xlat0.x = max(u_xlat0.x, _ShadowBrightness);
					    u_xlat0.x = min(u_xlat0.x, 1.0);
					    u_xlat0.x = u_xlat0.x * 0.899999976;
					    u_xlat1 = texture(_MainTex, vs_TEXCOORD0.xy);
					    u_xlat1 = vs_COLOR0 * u_xlat1 + (-_ShadowColor);
					    SV_Target0 = u_xlat0.xxxx * u_xlat1 + _ShadowColor;
					    return;
					}"
				}
			}
